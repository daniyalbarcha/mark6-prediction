...
class Mark6Scraper:
    def __init__(self, base_url="https://www.pilio.idv.tw/ltohk/list.asp"):
    ...
    def get_total_pages(self):
        ...
        try:
            ...
            records_per_page = sum(1 for row in table_rows if len(row.find_all('td')) == 3)  # Mark 6 has 3 columns
            ...
            return 140  # Fallback to known value for Mark 6
        ...
        for retry in range(max_retries):
            try:
                ...
                for row in soup.find_all('tr'):
                    ...
                    if len(cells) == 3:  # Mark 6 has 3 columns: date, numbers, extra
                        date_text = cells[0].get_text(strip=True)
                        numbers_text = cells[1].get_text(strip=True)
                        extra_text = cells[2].get_text(strip=True)
                        ...
                        if date_match:
                            ...
                            extra = self.clean_numbers(extra_text)
                            
                            if len(numbers) == 6 and len(extra) == 1:  # Ensure we have exactly 6 main numbers and 1 extra
                                data_rows.append({
                                    'date': date,
                                    'number1': numbers[0],
                                    'number2': numbers[1],
    ...
    def scrape_historical_data(self, start_page=1, end_page=None):
        ...
        print(f"\nScraping {total_pages} pages of historical Mark 6 data (Total available: {self.total_pages} pages)")
    ...
    def save_intermediate_results(self, data_list, current_page):
        ...
        if data_list:
            ...
            filename = f'mark6_history_intermediate_p{current_page}.csv'
            intermediate_data.to_csv(filename, index=False)
            print(f"\nSaved intermediate results up to page {current_page}")

    def save_to_csv(self, data, filename='mark6_history.csv'):
    ...
    def get_next_draw_date(self):
        """Get the next draw date based on the schedule (Tue/Thu/Sat)"""
        today = datetime.now()
        days_ahead = 0
        
        # Check next 7 days to find the next draw day
        for i in range(7):
            check_date = today + timedelta(days=i)
            if check_date.weekday() in [1, 3, 5]:  # Tuesday(1), Thursday(3), Saturday(5)
...
if __name__ == "__main__":
    ...
    if not historical_data.empty:
        ...
        print(f"Successfully scraped {len(historical_data)} Mark 6 lottery results") 